...
namespace lab5
{
    class Transport
    {
        public delegate void CarState(string message);
        public event CarState StartCar;

        public delegate int Distance(int speed, int time);
        ...
        public void KeyTurning(bool state)
        {
            if(engine == false && state == false)
            {
                StartCar?.Invoke("What is dead cannot die");
            }
            else if(engine == true && state == true)
            {
                StartCar?.Invoke("It's already started");
            }
            else
            {
                engine = state;
                StartCar?.Invoke("Operation successful");
            }
        }
