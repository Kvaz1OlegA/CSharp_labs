...
namespace lab6
{
    class Program
    {
            ...
            foreach (Scientist scientist1 in nominamted)
            {
                if(counter == prize)
                {
                    foreach (Scientist scientist in scientists)
                    {
                        ...
                        {
                            scientist.AddMoney += DisplayMessage;
                            scientist.Award(money);
                            Console.WriteLine(string.Format("\n{0} {1} become the Nobel Prize winner", scientist.Name, scientist.Surname));
                            scientist.AddMoney -= DisplayMessage;
                            return;
                        }
                    }
        ...
        delegate void Show(List<Scientist> transports);

        static Show PrintMenu = delegate (List<Scientist> scientists)
        {
            foreach (Scientist scientist in scientists)
            {
